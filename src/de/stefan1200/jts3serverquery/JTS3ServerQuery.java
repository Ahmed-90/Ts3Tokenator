 ...
 * @version 1.0.10 (17.06.2014)
 */
public class JTS3ServerQuery
{
	/**
	 * Setting DEBUG to <code>true</code> will write every exception into an error log file and write the communication log file.
	 ...
	 * This will be used only if DEBUG is set to <code>true</code>.
	 * In the error log file you can see any exceptions thrown by this library.
	 * @since 1.0.10
	 * @see JTS3ServerQuery#DEBUG
	 */
		...
		if (socketQuery.isConnected())
		{
			try
			{
				...
				catch (EOFException eof)
				{
					writeErrLog(eof);
					closeTS3Connection();
					saveLastError("Exception connectTS3Query(): " + eof.toString());
					return false;
				}
				catch (Exception e)
				{
	...
	{
		return selectVirtualServer(serverID, false, false);
	}
	...
	{
		return selectVirtualServer(server, selectPort, false);
	}
	 ...
	 * @see JTS3ServerQuery#getLastError()
	 */
	public boolean selectVirtualServer(int server, boolean selectPort, boolean virtual)
	{
		resetLastError();
	...
	public HashMap<String, String> doCommand(String command)
	{
		...
		if (command.startsWith("clientmove ") || command.startsWith("channeldelete "))
		{
